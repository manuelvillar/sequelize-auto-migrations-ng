...
function pruneOldMigFiles(revision, migrationsDir, options) {
  fs.readdir(migrationsDir, (err, files) => {
    if (options.debug && err) console.error(`Can't read dir: ${err}`);
    files.forEach((file) => {
      if (file.split('-')[0] === revision.toString()) {
        fs.unlink(`${migrationsDir}/${file}`, (error) => {
          ...
          } else if (options.verbose) console.log(`Successfully deleted ${file}`);
        });
      }
    });
  });
...
`;
  ...
  name = name.replace(' ', '_');
  const filename = path.join(migrationsDir, `${revision + (
    (name != '')
      ? `-${name}`
      : '')}.js`);
